...
from llama_index.core.vector_stores.types import VectorStoreQueryMode, VectorStoreQuerySpec
...
from prompts import system_prompt, FACT_CONDENSE_PROMPT, FACT_EXTRACT_PROMPT
from tracing_setup import init_phoenix_tracing
...
class SmartAutoRetriever(VectorIndexAutoRetriever):
    ...
    async def aretrieve_with_spec(self, str_or_query_bundle: str | QueryBundle) -> Tuple[List[NodeWithScore], VectorStoreQuerySpec]:
...
class RouteDecision(BaseModel):
    reasoning: str = Field(description="Μια σύντομη εξήγηση (concise rationale) για την απόφαση δρομολόγησης.")
...
class RouteAndCondenseDecision(BaseModel):
    reasoning: str = Field(description="Μια σύντομη εξήγηση (concise rationale) για την απόφαση δρομολόγησης και σύνοψης (contextualization) του ερωτήματος")
    route: Literal["rag", "general"] = Field(
...
class RelevanceEvaluation(BaseModel):
    reasoning: str = Field(description="Σύντομη εξήγηση για το αν τα έγγραφα περιέχουν την απάντηση.")
    is_relevant: bool = Field(description="True αν τα έγγραφα μπορούν να απαντήσουν την ερώτηση, αλλιώς False.")
...
class RAGWorkflow(Workflow):
    ...
    async def _stream_response_and_update_state(self, raw_stream: ChatResponseAsyncGen, role: MessageRole, on_complete_callback=None):
    ...
    @step
    async def route_and_condense_query(self, ctx: Context, ev: RouteEvent) -> CheckCacheEvent | SynthesizeGeneralEvent:
        ...
        conversation_history = [m for m in chat_history if m.role != MessageRole.SYSTEM][:-1]
        # last 4 messages
    ...
    @step
    async def check_semantic_cache(self, ctx: Context, ev: CheckCacheEvent) -> RetrieveEvent | StopEvent:
        if self.cache_retriever:
            cache_results = await self.cache_retriever.aretrieve(ev.condensed_query)

            if cache_results and cache_results[0].score > settings.semantic_cache_threshold:
                cached_answer = cache_results[0].metadata.get("answer")

                ctx.write_event_to_stream(UIProgressEvent(
                    step_name='Semantic Cache', msg='Η απάντηση ανασύρθηκε άμεσα από την προσωρινή μνήμη (Cache)!'
                ))

                logger.info(
                    "Cache Hit", 
                    extra={"score": cache_results[0].score, "cached_text": cache_results[0].text}
                )
                await self.memory.aput(ChatMessage(role=MessageRole.SYSTEM, content=cached_answer))
                ...
                async def cached_response_generator():
                    for word in cached_answer.split(" "):
                        ...
                        await asyncio.sleep(0.005) # make streaming more natural and not so bursty
    ...
    @step
    async def retrieve_and_evaluate(self, ctx: Context, ev: RetrieveEvent) -> SynthesizeEvent | RewriteQueryEvent | SynthesizeFallbackEvent:
        ...
        if not nodes:
            logger.warning("AutoRetriever failed. Falling back to VectorIndexRetriever.")
        ...
        if used_filters:
            filter_details = ", ".join([f"{f.key} {f.operator.value} {f.value}" for f in spec.filters])
            logger.info("AutoRetriever used metadata filters", extra={"filters": spec.filters, "filters_details": filter_details})
            valid_nodes = nodes # No score threshold if filters are heavily applied
        else:
            
            valid_nodes = [n for n in nodes if n.score >= settings.semantic_threshold][:settings.nodes_sent_to_llm]
            logger.debug("Pure Semantic Search executed. No filters applied.", 
    ...
    async def _handle_retry(self, ctx: Context, query: str) -> RewriteQueryEvent | SynthesizeFallbackEvent:
    ...
    async def synthesize(self, ctx: Context, ev: SynthesizeEvent) -> StopEvent:
        ...
        ctx.write_event_to_stream(UIProgressEvent(
            step_name='Σύνθεση απάντησης',
            msg='Γίνεται σύνθεση της απάντησης.'
        ))
        ...
        stream_gen = self._stream_response_and_update_state(
                raw_response_stream, MessageRole.ASSISTANT, on_complete_callback=cache_callback
        )
        
        return StopEvent(result=stream_gen)
        
    @step
    async def synthesize_general(self, ctx: Context, ev: SynthesizeGeneralEvent) -> StopEvent:
        ...
        stream_gen = self._stream_response_and_update_state(raw_response_stream, MessageRole.ASSISTANT)

        return StopEvent(result=stream_gen)
    
    @step
    async def synthesize_fallback(self, ctx: Context, ev: SynthesizeFallbackEvent) -> StopEvent:
